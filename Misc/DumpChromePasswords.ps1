...
foreach ($datarow in $data.Tables.rows)
{
    ...
    $row | Add-Member -Name Password -MemberType NoteProperty -Value (([System.Text.Encoding]::ASCII.GetString([System.Security.Cryptography.ProtectedData]::Unprotect($datarow.password_value,$null,[System.Security.Cryptography.DataProtectionScope]::CurrentUser))))
    $arrExp += $row
}
