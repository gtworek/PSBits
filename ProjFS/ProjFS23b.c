...
#define VFILENAMEW L"test.ps1"
#define VFILESIZE 1024
#define POWERSHELLIMGW L"\\Device\\HarddiskVolume5\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe"
...
)
{
	...
	{
		if (0 == wcscmp(callbackData->TriggeringProcessImageFileName,POWERSHELLIMGW))
		{
			(void)swprintf(Buf, length, L"%s", L"\xFEFFWrite-Host \"`r`nAll your base are belong to us.\" -ForegroundColor Red");
		}
		else
		{
			(void)swprintf(Buf, length, L"%s", L"\r\n# Please disperse. Nothing to see here.\r\n");
		}
	}
